...
import '../../cards/saleslist/sales_last_7_days_card.dart';
...
class Top5SellingProducts7DaysDialog extends StatefulWidget {
  @override
  _Top5SellingProducts7DaysDialogState createState() => _Top5SellingProducts7DaysDialogState();
}

class _Top5SellingProducts7DaysDialogState extends State<Top5SellingProducts7DaysDialog> {
  List<Top5SellingProducts7Days> salesItems = [];
  bool isLoading = true;
  ...
  Future<void> _runBackgroundTask() async {
    try {
      ...
      final response = await http.post(
        Uri.parse('$baseUrl/auth/sales/top5sellingProductsD7'),
        headers: {
                ...
                return Expanded(
                  child: ListView.builder(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      return Top5sellingProducts7DaysCard(
  ...
  List<Top5SellingProducts7Days> parseSalesItems(String jsonStr) {
    final parsed = json.decode(jsonStr);
    final salesList = List<Map<String, dynamic>>.from(parsed['sales']);

    return salesList.map((item) {
      return Top5SellingProducts7Days(
        item['txnSale'] ?? 0,
        item['txnCount'] ?? 0,
        item['product'] ?? '',
      );
