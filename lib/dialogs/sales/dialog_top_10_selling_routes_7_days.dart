...
import 'package:vipapp/model/salestab/sales_tab_top_5_selling_products_7_days.dart';

import '../../cards/saleslist/top_10_selling_routes_7_days_card.dart';
import '../../cards/saleslist/top_5_selling_products_7_days_card.dart';
import '../../model/salestab/sales_top_10_selling_routes_7_days.dart';
import '../../pref/peference_manager.dart';
import '../../utils/Network_info_util.dart';
import 'package:http/http.dart' as http;

class Top10SellingRoutes7DaysDialog extends StatefulWidget {
  @override
  _Top10SellingRoutes7DaysDialogState createState() => _Top10SellingRoutes7DaysDialogState();
}

class _Top10SellingRoutes7DaysDialogState extends State<Top10SellingRoutes7DaysDialog> {
  List<Top10SellingRoutes7Days> salesItems = [];
  bool isLoading = true;
  ...
  Future<void> _runBackgroundTask() async {
    try {
      ...
      final response = await http.post(
        Uri.parse('$baseUrl/auth/sales/top10sellingRoutesD7'),
        headers: {
  ...
  List<Top10SellingRoutes7Days> parseSalesItems(String jsonStr) {
    final parsed = json.decode(jsonStr);
    final salesList = List<Map<String, dynamic>>.from(parsed['sales']);

    return salesList.map((item) {
      return Top10SellingRoutes7Days(
        item['txnSale'] ?? 0,
        item['txnCount'] ?? 0,
        item['route'] ?? '',
        item['ferryName'] ?? '',
